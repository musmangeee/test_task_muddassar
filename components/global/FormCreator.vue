<template>
	<v-row :id="ref" v-bind="getRow" v-resize.quiet="onResize">
		<!-- FORM-BASE TOP SLOT -->
						...
						<slot :name="getTypeItemSlot(obj)" :obj="obj">
							...
							<slot :name="getKeyItemSlot(obj)" :obj="obj">
								...
								<template v-else-if="obj.schema.type === 'group'">
									<div v-bind="bindSchema(obj)" @click="onEvent($event, obj)">
		...
		methods: {
			...
			emitValue(emit, val) {
				...
				if (change.indexOf(emit) > -1)
					this.parent.$emit(this.getEventName("change"), val) // listen to 'input|click'
				...
				if (mouse.indexOf(emit) > -1)
					this.parent.$emit(this.getEventName("mouse"), val) // listen to 'mouseenter|mouseleave  '
				if (display.indexOf(emit) > -1)
					this.parent.$emit(this.getEventName("display"), val) // listen to 'resize|swipe|intersect'
			...
			rebuildArrays(model, schema) {
				...
				if (!model)
					throw `Property 'model' is null or undefined. Use '<FormCreator :model="myModel" :schema="mySchema" />'. myModel must be at least an empty Object.`

				// noise if async loading
				if (isEmpty(schema) && isEmpty(model)) {
					console.warn(
						`At least one of the properties 'model' or 'schema' in <FormCreator /> must be at least an empty Object. Ignore this Warning on async loading 'model' or 'schema' `
					)
					// return
				}
